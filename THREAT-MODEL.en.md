...
| Key leakage through backups | `allowBackup="false"`; neither Android cloud backup nor the legacy adb backup path is allowed for the app. |
| Device theft or physical extraction | Android 10 and later require file-based encryption. App-private data cannot be decrypted without the device's lock credential. |
...
| OpenAI's handling of submitted API data | OpenAI is the provider selected by the user. OpenAI states that API inputs and outputs are not used for training by default and that some API data may be retained for abuse monitoring, but policies and endpoint-specific controls change. Check OpenAI's current first-party data-control documentation yourself before deciding whether the provider fits your requirements. |
...
Current first-party references for OpenAI API data handling:
